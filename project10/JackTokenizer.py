...
class JackTokenizer:
        ...
        while self.has_more_tokens():
            ...
            while len(self._current_line):
                ...
                if token_type == "SYMBOL":
                    ...
                    token = "keyword"
                if tag != '':
                    self._token_lst.append((tag, token))
    ...
    def int_val(self) -> int:
        ...
        """
        return int(self._current_command)
    ...
    def gets_comment_length(self, i) -> int:
        """
        calculate the length of the comment in order to delete it
        """
        while "*/" not in self._input_lst[i]:
            i += 1
        i += 1
        return i
    ...
    def change_symbol(self, symbol: str) -> None:
        """
        changes the symbol if it's one of the following <, >, &
        @param symbol: symbol to change
        """
        if symbol == "<":
            self._symbol = "&lt;"
        elif symbol == ">":
